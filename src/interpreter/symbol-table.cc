...
SymbolAttr& SymbolTable::SetValue(const std::string& name, bool global) {
  ...
  if (it != map_.end()) {
    ...
    SymbolIterator it_symbol = map_.insert(
        it, std::pair<std::string, SymbolAttr>(name, std::move(symbol)));
...
std::map<std::string, std::shared_ptr<Object>> SymbolTable::SymMap() {
  ...
  for (auto& e : map_) {
    map.insert(
        std::pair<std::string, ObjectPtr>(e.first, e.second.SharedAccess()));
  }
...
#define ALOC_TYPE(NAME, FNAME)                              \
  ...
  symbol_table.InsertSysEntry(                              \
      static_cast<const FNAME##Type&>(*type_##NAME).name(), \
      std::move(symbol_##NAME));
...
void AlocTypes(SymbolTableStack& symbol_table) {
  ...
  symbol_table.InsertSysEntry(
      static_cast<const RootObjectType&>(*type_obj_root).name(),
      std::move(symbol_root));
  ...
  ObjectPtr type_glob = obj_factory.NewGlobType();
  SymbolAttr symbol_glob(type_glob, true);
  symbol_table.InsertSysEntry(static_cast<const GlobType&>(*type_glob).name(),
                              std::move(symbol_glob));
  ...
  symbol_table.InsertSysEntry(
      static_cast<const CmdIterType&>(*type_cmd_iter).name(),
      std::move(symbol_cmd_iter));

  ObjectPtr type_glob_iter = obj_factory.NewGlobIterType();
  SymbolAttr symbol_glob_iter(type_glob_iter, true);
  symbol_table.InsertSysEntry(
      static_cast<const GlobIterType&>(*type_glob_iter).name(),
      std::move(symbol_glob_iter));

  ObjectPtr type_file_iter = obj_factory.NewFileIterType();
  SymbolAttr symbol_file_iter(type_file_iter, true);
  symbol_table.InsertSysEntry(
      static_cast<const FileIterType&>(*type_file_iter).name(),
      std::move(symbol_file_iter));

  ObjectPtr type_array_iter = obj_factory.NewArrayIterType();
  SymbolAttr symbol_array_iter(type_array_iter, true);
  symbol_table.InsertSysEntry(
      static_cast<const ArrayIterType&>(*type_array_iter).name(),
      std::move(symbol_array_iter));

  ObjectPtr type_tuple_iter = obj_factory.NewTupleIterType();
  SymbolAttr symbol_tuple_iter(type_tuple_iter, true);
  symbol_table.InsertSysEntry(
      static_cast<const TupleIterType&>(*type_tuple_iter).name(),
      std::move(symbol_tuple_iter));

  ObjectPtr type_range_iter = obj_factory.NewRangeIterType();
  SymbolAttr symbol_range_iter(type_range_iter, true);
  symbol_table.InsertSysEntry(
      static_cast<const RangeIterType&>(*type_range_iter).name(),
      std::move(symbol_range_iter));

  ObjectPtr type_map_iter = obj_factory.NewMapIterType();
  SymbolAttr symbol_map_iter(type_map_iter, true);
  symbol_table.InsertSysEntry(
      static_cast<const MapIterType&>(*type_map_iter).name(),
      std::move(symbol_map_iter));
  ...
  symbol_table.InsertSysEntry(
      static_cast<const ModuleType&>(*type_module).name(),
...
std::tuple<std::shared_ptr<Object>, bool> SymbolTableStack::LookupObj(
